...
export class PaymentsTools implements ITools {
  ...
  constructor() {
    this.w3 = new Web3(
      new Web3.providers.HttpProvider(rpcUrls["base-sepolia"])
    );
  ...
  async signTransaction(
    ...
    const amountWei = BigInt(
      Math.floor(amount * 10 ** currencyDetails.decimals)
    );
  ...
  async sendPayment(
    ...
    try {
      ...
      if (currencyDetails.isNative) {
        const receipt = await this.w3.eth.sendSignedTransaction(rawTransaction);
        return {
        ...
        if (!contractAddress) {
          return {
            resultMessage:
              "Token contract address not found for this payment method",
            hash: "",
          };
        }

        const tokenContract = new this.w3.eth.Contract(
          ERC20_ABI,
          contractAddress
        );
  ...
  async verifyPayment(
    ...
    try {
      ...
      if (currencyDetails.isNative) {
        ...
        if (
          transaction.to?.toLowerCase() !== buyerWalletAddress.toLowerCase()
        ) {
          return {
            success: false,
        ...
        if (!contractAddress) {
          return {
            success: false,
            message: "Token contract address not found for this payment method",
          };
        }
        ...
        const decodedInput = tokenContract.methods
          .transfer(buyerWalletAddress, expectedAmount.toString())
          .encodeABI();
